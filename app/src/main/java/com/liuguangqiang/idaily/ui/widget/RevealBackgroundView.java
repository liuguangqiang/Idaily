...
public class RevealBackgroundView extends View {
    ...
    public static final int BACK_FILL_STARTED = 3;
    public static final int BACK_FINISHED = 4;

    private static final Interpolator INTERPOLATOR = new AccelerateInterpolator();
    private static final int FILL_TIME = 400;
    private static final int FILL_TIME_END = 600;
    ...
    public void startFromLocation(int[] tapLocationOnScreen) {
        ...
        startLocation = tapLocationOnScreen;
    ...
    public void back() {
        if (startLocation != null) {
            backFromLocation(startLocation);
        }
    }

    public void backFromLocation(int[] tapLocationOnScreen) {
        changeState(BACK_FILL_STARTED);
        startLocationX = tapLocationOnScreen[0];
        startLocationY = tapLocationOnScreen[1];
        revealAnimator = ObjectAnimator.ofInt(this, "currentRadius", getWidth() + getHeight(), 0).setDuration(FILL_TIME_END);
        revealAnimator.setInterpolator(INTERPOLATOR);
        revealAnimator.addListener(new AnimatorListenerAdapter() {
            @Override
            public void onAnimationEnd(Animator animation) {
                setVisibility(View.GONE);
                changeState(BACK_FINISHED);
            }
        });
        revealAnimator.start();
    }
